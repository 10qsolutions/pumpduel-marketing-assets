...
export const pageFiles = ['index.html','styles.css','app.js','catalog.json','global.html','global-pack.json','global-pack.css','global-pack.js','global-guide.md','fonts/BarlowCondensed-Black.ttf','fonts/OFL.txt'];
...
export async function validateFiles(root) {
  ...
  const globalPack = JSON.parse(await readFile(resolve(root,'global-pack.json'),'utf8'));
  ...
  if (globalPack.status !== 'HOLD_FOR_WORKING_LINKS' || globalPack.postingApproved !== false || !Array.isArray(globalPack.assets) || !globalPack.assets.length || !Array.isArray(globalPack.posts)) throw new Error('Invalid global launch draft');
  const allAssets = [...catalog.assets,...globalPack.assets];
  const ids = new Set();
  const paths = new Set();
  ...
  for (const post of [...catalog.posts,...globalPack.posts]) {
  ...
  for (const file of publicFiles) {
    ...
    const asset = allAssets.find(item => item.file === file);
    ...
    if (/\.(html|css|js|json|md)$/.test(file) && privateText.test(buffer.toString('utf8'))) throw new Error(`Private data pattern in public file: ${file}`);
  }
  return {catalog, globalPack, publicFiles};
}
...
export function contentType(file) {
  return mediaTypes[extname(file)] || {'.html':'text/html; charset=utf-8','.js':'text/javascript; charset=utf-8','.css':'text/css; charset=utf-8','.json':'application/json; charset=utf-8','.ttf':'font/ttf','.txt':'text/plain; charset=utf-8','.md':'text/plain; charset=utf-8'}[extname(file)] || 'application/octet-stream';
}
